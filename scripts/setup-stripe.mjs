...
async function ensurePrice(plan, nombreSufijo, cantidad, nombreCompleto) {
  ...
  const product = await stripe.products.create({
    name: `Kreditools ${plan.nombre} ${nombreSufijo === 'Mensual' ? 'Mensual' : 'Anual'}`,
...
async function main() {
  ...
  for (const plan of plans) {
    ...
    const mensualId = await ensurePrice(plan, 'Mensual', plan.precioMensual, `Kreditools ${plan.nombre} Mensual`);
    const anualId = await ensurePrice(plan, 'Anual', plan.precioAnual ?? plan.precioMensual * 10, `Kreditools ${plan.nombre} Anual`);
