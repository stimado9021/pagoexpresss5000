...
const from = process.env.EMAIL_FROM || 'Kreditools <noreply@kreditools.shop>'
...
export async function sendCredenciales({
  ...
  return sendEmail({
    to,
    subject: `Tus credenciales de acceso a Kreditools${rol ? ` (${rol})` : ''}`,
    html: layoutHtml(`
      <h1 style="font-size:20px;margin:0 0 12px;">¡Hola, ${nombre}!</h1>
      <p style="margin:0 0 16px;">Tu usuario en Kreditools quedó listo. Estas son tus credenciales de acceso:</p>
    ...
    <div style="max-width:520px;margin:0 auto;background:#0f172a;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);">
      <div style="background:#14532d;padding:24px 32px;">
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <span style="color:#f5f5f4;font-size:17px;font-weight:700;">Kreditools</span>
        </div>
      </div>
      ...
      <div style="padding:16px 32px;border-top:1px solid rgba(255,255,255,0.08);color:#a8a29e;font-size:12px;text-align:center;">
        &copy; ${new Date().getFullYear()} Kreditools &bull; Gestión de préstamos
      </div>
    </div>
  </div>`
}
